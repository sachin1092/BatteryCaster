...
public class HistoryItem implements Serializable, Parcelable {
	...
	static final int MOST_INTERESTING_STATES = STATE_BATTERY_PLUGGED_FLAG
			| STATE_SCREEN_ON_FLAG | STATE_GPS_ON_FLAG
			| STATE_PHONE_IN_CALL_FLAG;
	...
	public HistoryItem(Long time, Byte cmd, Byte batteryLevel,
			Byte batteryStatusValue, Byte batteryHealthValue,
			Byte batteryPlugTypeValue, String batteryTemperatureValue,
			String batteryVoltageValue, Integer statesValue) {
		m_time = time;
	...
	/**
	 * Returns the Long time of the HistoryItem as string
	 * 
	 * @return the Long time as sting
	 */
	public String getLongTime() {
		return DateUtils.format("MM/dd/yyyy hh:mm:ss a", m_time + m_offset);

	}
	
	/**
	 * Returns the Calendar instance of the HistoryItem
	 * 
	 * @return the Calendar instance
	 */	
	public Calendar getCalendarInstance(){
		return DateUtils.getCalendar("MM/dd/yyyy hh:mm:ss a", m_time + m_offset);
	}
	 ...
	 * calculated as the difference of the most recent sample from the stats and
	 * the current time at the point of reading the stats. This norms the last
	 * sample to the time the data is queried
	 * 
