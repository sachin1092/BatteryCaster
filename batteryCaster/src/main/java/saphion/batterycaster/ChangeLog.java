...
public class ChangeLog {
	 ...
	 */
	private static final String DEFAULT_CSS = "@font-face { font-family: \"MyFont\";src: url('file:///android_asset/Dosis-Book.ttf');}"
			+ "h1 { margin-left: 0px; font-size: 1.2em; font-family:\"MyFont\";}"
			+ "li { margin-left: 0px;font-family:\"MyFont\";}"
			+ "ul { padding-left: 2em;font-family:\"MyFont\";}";

	/**
	 * Context that is used to access the resources and to create the ChangeLog
	 * dialogs.
	 */
	...
	/**
	 * Last version code read from {@code SharedPreferences} or
	 * {@link #NO_VERSION}.
	 */
	...
	/**
	 * Create a {@code ChangeLog} instance using the default
	...
	public ChangeLog(Context context) {
		this(context, context.getSharedPreferences("saphion.batterycaster_preferences",
	...
	/**
	 * Create a {@code ChangeLog} instance using the supplied
	 ...
	 *            ChangeLog dialogs.
	 * @param preferences
	 *            {@code SharedPreferences} instance that is used to persist the
	 *            last version code.
	 * @param css
	 *            CSS styles used to format the change log (excluding
	...
	public ChangeLog(Context context, SharedPreferences preferences, String css) {
		...
		try {
			...
			Log.e(LOG_TAG, "Could not get version information from manifest!",
					e);
		}
	}
	 ...
	 * 
	 * @return The version code of the last installation of this app (as
	 *         described in the former manifest). This will be the same as
	 *         returned by {@link #getCurrentVersionCode()} the second time this
	 ...
	 * 
	 * @return {@code true} if this version of your app is started the first
	 *         time.
	 */
	 ...
	 * 
	 * @return {@code true} if your app including {@code ChangeLog} is started
	 *         the first time ever. Also {@code true} if your app was
	 ...
	 *         first run of your app including {@code ChangeLog} then the full
	 *         log dialog is show.
	 */
	 ...
	 * 
	 * @param full
	 *            If this is {@code true} the full change log is displayed.
	...
	protected AlertDialog getDialog(boolean full) {
		...
		View mView = LayoutInflater.from(mContext).inflate(R.layout.changelog,
				null, true);

		WebView wv = (WebView) mView.findViewById(R.id.wvChangeLog);
		...
		((TextView) mView.findViewById(R.id.tvchangeLogTitle))
				.setText(full ? "Change Log" : "What's New");
		 ...
		 * builder.setTitle(
		 * 
		 * full ? "Change Log" : "What's New").setView(wv).setCancelable(false)
		 * // OK button .setPositiveButton("OK", new
		 * DialogInterface.OnClickListener() {
		 * 
		 * @Override public void onClick(DialogInterface dialog, int which) { //
		 * The user clicked "OK" so save the current // version code as //
		 * "last version code". updateVersionInPreferences(); } });
		 * 
		 * if (!full) { // Show "More…" button if we're only displaying a
		 * partial change // log. builder.setNegativeButton("More�", new
		 * DialogInterface.OnClickListener() {
		 * 
	...
	protected void updateVersionInPreferences() {
		SharedPreferences sp = mContext.getSharedPreferences("saphion.batterycaster_preferences",
				Context.MODE_MULTI_PROCESS);
	 ...
	 * 
	 * @param full
	 *            If this is {@code true} the full change log is returned.
	 *            Otherwise only changes for versions newer than the last
	...
	private String getLog(boolean full) {
		...
		try {
			XmlPullParser xml = XmlPullParserFactory.newInstance()
					.newPullParser();
		...
		XmlResourceParser resXml = mContext.getResources().getXml(
				R.xml.changelog);
		...
		String versionFormat = resources
				.getString(R.string.changelog_version_format);
		...
		for (Integer version : versions) {
			...
			// Use release information from localized change log and fall back
			// to the master file
			// if necessary.
	 ...
	 *            version code.
	 * @param changelog
	 *            The {@code SparseArray} to add a new {@link ReleaseItem}
	 *            instance to.
	 * 
