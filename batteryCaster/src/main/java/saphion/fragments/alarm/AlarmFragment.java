...
public class AlarmFragment extends Fragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mCursorLoader = getActivity().getSupportLoaderManager().initLoader(0,
				null, this);
		try {
			if (!created)
		...
		final View v = inflater.inflate(R.layout.alarm_listview, container,
				false);
		...
		if (savedState != null) {
			...
			mRingtoneTitleCache = savedState
					.getBundle(KEY_RINGTONE_TITLE_CACHE);
		...
		mCollapseInterpolator = new DecelerateInterpolator(
				COLLAPSE_DECELERATION);
		...
		mFadeOut = AnimatorInflater
				.loadAnimator(getActivity(), R.anim.fade_out);
		...
		mAdapter = new AlarmItemAdapter(getActivity(), expandedIds,
	...
	private void showUndoBar() {
		...
		if (mDeletedAlarm != null) {
			...
			mMessageBar
					.show("Alarm Deleted", "UNDO", R.drawable.ic_action_undo);
		}
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putLongArray(KEY_SELECTED_ALARMS,
				mAdapter.getSelectedAlarmsArray());
		...
		outState.putBundle(KEY_PREVIOUS_DAY_MAP,
				mAdapter.getPreviousDaysOfWeekMap());
	...
	private void showLabelDialog(final Alarm alarm) {
		...
		(new Handler()).postDelayed(new Runnable() {
			...
			public void run() {
				picker.dispatchTouchEvent(MotionEvent.obtain(
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		final Context context = AlarmFragment.this.getActivity()
				.getApplicationContext();
		final AsyncTask<Void, Void, Void> updateTask = new AsyncTask<Void, Void, Void>() {

			@Override
			protected Void doInBackground(Void... parameters) {
				ArrayList<AlarmItems> prevAlarms = AlarmPreference
						.retAlarms(getBaseContext());

				for (AlarmItems alarm : prevAlarms)

					if (context != null && alarm != null) {
						ContentResolver cr = context.getContentResolver();
						if (!Alarm.contains(context.getContentResolver(),
								new Alarm(alarm)))
			...
			protected void onPostExecute(Void result) {
				...
				getActivity()
						...
						.putBoolean("FirstAfterUpdate", false).commit();
			}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			case REQUEST_CODE_RINGTONE:
				...
				break;
			default:
				Log.d("Unhandled request code in onActivityResult: "
						+ requestCode);
			}
		}
	}
	...
	public class AlarmItemAdapter extends CursorAdapter {
		...
		@SuppressLint("NewApi")
		public AlarmItemAdapter(Context context, long[] expandedIds,
				long[] repeatCheckedIds, long[] selectedAlarms,
			...
			mRobotoBold = Typeface
					.create("sans-serif-condensed", Typeface.BOLD);
			mRobotoNormal = Typeface.create("sans-serif-condensed",
					Typeface.NORMAL);
			...
			mCollapseExpandHeight = (int) res
					.getDimension(R.dimen.collapse_expand_height);
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				boolean badConvertView = convertView
						.findViewById(R.id.digital_clock) == null;
					...
					for (int i = 0; i < list.getChildCount(); i++) {
						...
						if (startTop == null) {
							int childHeight = child.getHeight()
									+ list.getDividerHeight();
							startTop = top
									+ (i > 0 ? childHeight : -childHeight);
						}
						...
						if (startTop != top) {
							...
							animate(child).setDuration(ANIMATION_DURATION)
									.translationY(0);
		...
		private void setNewHolder(View view) {
			...
			holder.clickableLabel = (TextView) view
					.findViewById(R.id.edit_label);
            holder.clickableLabel.setTypeface(mRobotoNormal);
			holder.repeatDays = (LinearLayout) view
					.findViewById(R.id.repeat_days);
			holder.repeatCharge = (LinearLayout) view
					.findViewById(R.id.repeat_charge);
			...
			for (int i = 0; i < 7; i++) {
				final ViewGroup viewgroup = (ViewGroup) mFactory.inflate(
						R.layout.day_button, holder.repeatDays, false);
				final ToggleButton button = (ToggleButton) viewgroup
						.getChildAt(0);
			...
			for (int i = 0; i < 2; i++) {
				final ViewGroup viewgroup = (ViewGroup) mFactory.inflate(
						R.layout.charge_button, holder.repeatCharge, false);
				final ToggleButton button = (ToggleButton) viewgroup
						.getChildAt(0);
			...
			holder.ringtone = (TextView) view
					.findViewById(R.id.choose_ringtone);
		...
		public void bindView(final View view, Context context,
			...
			if (mSelectedAlarms.contains(itemHolder.alarm.id)) {
				itemHolder.alarmItem
						.setBackgroundColor(mBackgroundColorExpanded);
			...
			itemHolder.expandArea.setVisibility(expanded ? View.VISIBLE
					: View.GONE);
			...
			final String daysOfWeekStr = alarm.daysOfWeek.toString(
					AlarmFragment.this.getActivity(), false);
			if (daysOfWeekStr != null && daysOfWeekStr.length() != 0) {
				itemHolder.daysOfWeek.setText(daysOfWeekStr);
				itemHolder.daysOfWeek
						.setContentDescription(alarm.daysOfWeek
								.toAccessibilityString(AlarmFragment.this
										.getActivity()));
			...
			if (alarm.label != null && alarm.label.length() != 0) {
				...
				itemHolder.label.setContentDescription(mContext.getResources()
						.getString(R.string.todo) + " " + alarm.label);
		...
		private void bindExpandArea(final ItemHolder itemHolder,
			...
			itemHolder.repeat.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View view) {
					...
					if (checked) {
						...
						final int bitSet = mPreviousDaysOfWeekMap.getInt(""
								+ alarm.id);
			...
			for (int i = 0; i < 7; i++) {
				...
				itemHolder.dayButtonParents[i]
						.setOnClickListener(new View.OnClickListener() {
							@Override
							public void onClick(View view) {
								itemHolder.dayButtons[buttonIndex].toggle();
								final boolean checked = itemHolder.dayButtons[buttonIndex]
										.isChecked();
								...
								if (checked) {
									...
									if (!alarm.daysOfWeek.isRepeating()) {
										itemHolder.repeatDays
												.setVisibility(View.GONE);
										itemHolder.repeat
												.setTextColor(mColorDim);
										mRepeatChecked.remove(alarm.id);

										// Set history to no days, so it will be
										// everyday when repeat is
										// turned back on
										mPreviousDaysOfWeekMap.putInt(""
												+ alarm.id,
			...
			for (int i = 0; i < 2; i++) {
				...
				itemHolder.chargeButtonParents[i]
						.setOnClickListener(new View.OnClickListener() {
							@Override
							public void onClick(View view) {
								itemHolder.chargeButtons[buttonIndex].toggle();
								final boolean checked = itemHolder.chargeButtons[buttonIndex]
										.isChecked();
			...
			itemHolder.ringtone.setContentDescription(mContext.getResources()
					.getString(R.string.ringtone_description) + " " + ringtone);
		...
		private void updateDaysOfWeekButtons(ItemHolder holder,
		...
		private void expandAlarm(final ItemHolder itemHolder, boolean animate) {
			...
			if (!animate) {
				...
				final ViewTreeObserver observer = mAlarmsList
						.getViewTreeObserver();
				observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
					@Override
					public boolean onPreDraw() {
						...
						int collapseHeight = itemHolder.collapseExpandArea
								.getHeight() - hairlineHeight;
						animate(itemHolder.hairLine).setDuration(0)
								.translationY(-collapseHeight);
						return true;
					}
				});
				return;
			...
			observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
				...
				public boolean onPreDraw() {
					...
					final int collapseHeight = itemHolder.collapseExpandArea
							.getHeight();
					int hairlineHeight = itemHolder.hairLine.getHeight();
					final int hairlineDistance = collapseHeight
							- hairlineHeight;
					...
					animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
						...
						public void onAnimationUpdate(ValueAnimator animator) {
							...
							// parts of the layout.
							itemHolder.alarmItem.getLayoutParams().height = (int) (value
									* distance + startingHeight);
							FrameLayout.LayoutParams expandParams = (FrameLayout.LayoutParams) itemHolder.expandArea
									.getLayoutParams();
							expandParams.setMargins(0,
									(int) -((1 - value) * distance), 0,
									collapseHeight);
							animate(itemHolder.arrow).rotation(180 * value)
									.setDuration(0);
							animate(itemHolder.hairLine).setDuration(0)
									.translationY(-hairlineDistance * value);
					...
					animator.addListener(new AnimatorListener() {
						...
						public void onAnimationEnd(Animator animation) {
							...
							animate(itemHolder.arrow).rotation(180)
									.setDuration(0);
							animate(itemHolder.hairLine).setDuration(0)
									.translationY(-hairlineDistance);
		...
		private void collapseAlarm(final ItemHolder itemHolder, boolean animate) {
			...
			observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
				...
				public boolean onPreDraw() {
					...
					final int distance = endingHeight - startingHeight;
					int hairlineHeight = itemHolder.hairLine.getHeight();
					final int hairlineDistance = mCollapseExpandHeight
							- hairlineHeight;
					...
					animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
						...
						public void onAnimationUpdate(ValueAnimator animator) {
							...
							animate(itemHolder.arrow).rotation(
									180 * (1 - value)).setDuration(0);
							animate(itemHolder.hairLine).setDuration(0)
									.translationY(
											-hairlineDistance * (1 - value));
					...
					animator.addListener(new AnimatorListener() {
						...
						public void onAnimationEnd(Animator animation) {
							...
							itemHolder.expandArea.setVisibility(View.GONE);
							animate(itemHolder.arrow).rotation(0)
									.setDuration(0);
							animate(itemHolder.hairLine).setDuration(0)
									.translationY(0);
						}
