...
public class MyColorPickerView extends ActionBarActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra("color")) {
			...
			getSharedPreferences("saphion.batterycaster_preferences",
					...
					.edit()
					.putInt("color",
							getIntent().getIntExtra("color", 0xff33b5e5))
					.commit();
		} else {
			finish();
		}
	...
	public void onClick(View v) {
		if (v.getId() == R.id.new_color_panel) {
			...
			getSharedPreferences("saphion.batterycaster_preferences",
					...
					.putInt("color", mNewColor.getColor()).commit();

		}
		getSharedPreferences("saphion.batterycaster_preferences",
				Context.MODE_MULTI_PROCESS).edit()
				.putBoolean("fromcolor", true).commit();
		finish();
	}
