...
public class MessageBar {
	...
	public MessageBar(Activity activity) {
		ViewGroup container = (ViewGroup) activity
				.findViewById(android.R.id.content);
		View v = activity.getLayoutInflater().inflate(R.layout.mb__messagebar,
				container);
		init(v);
	}
	...
	private void show(Message message, boolean immediately) {
		...
		if (message == null || message.mMessage.isEmpty())
			return;
	...
	private static class Message implements Parcelable {
		...
		public Message(String message, String actionMessage, int actionIcon,
				Parcelable token) {
			mMessage = message;
